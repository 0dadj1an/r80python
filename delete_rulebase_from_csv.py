...
with open('rules.csv') as csvfile:
		...
		if row['type'] == "section_tittle":
			#delete section tittle
			section_response=r80_apis.delete_access_section_by_name(sid,url,layer,row['name'])
		if row['type'] == "rule":
			rule={}
			#source
			if '|' in row['source']:
			...
			rule['name']=row['name']
			#delete rule
			rule_response=r80_apis.delete_access_rule_by_rule_name(sid,url,layer,rule['name'])
			print rule_response
